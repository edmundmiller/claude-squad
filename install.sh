...
get_latest_version() {
    ...
    API_RESPONSE=$(curl -sS "https://api.github.com/repos/smtg-ai/agent-fleet/releases")
    if [ $? -ne 0 ]; then
...
extract_and_install() {
    ...
    mv "${tmp_dir}/agent-fleet${extension}" "$bin_dir/$INSTALL_NAME${extension}"
    rm -rf "$tmp_dir"
...
main() {
    ...
    RELEASE_URL="https://github.com/smtg-ai/agent-fleet/releases/download/v${VERSION}"
    ARCHIVE_NAME="agent-fleet_${VERSION}_${PLATFORM}_${ARCHITECTURE}${ARCHIVE_EXT}"
